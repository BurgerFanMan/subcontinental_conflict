...
class SquadEquipmentKit : Tracker {
	...
	protected void handleResultEvent(const XmlElement@ event) {
		//squad equipment kit notify_script keys. Add key here, then in .weapon file (projectile/key attribute)
		dictionary keys = {
			{"squad_kit_t2", "vest2.carry_item" },
			{"squad_kit_camo", "camouflage_suit.carry_item"}
			};
		...
		for (uint i = 0; i < keys.getKeys().size(); ++i) {
			if(keys.getKeys()[i] == eventKey){
				...
				keys.get(keys.getKeys()[i], newVest);
				}
		}
		...
		if (containsEventKey) {
			...
			//effect radius
			float range = 30.0;
			//the minimum xp each soldier needs to be given the vest
			float minXp = 0.0;
			...
			if (character !is null) {
				...
				for (uint i = 0; i < total_soldier_count; ++i) {
					...
					if (targetKeys.find(soldierClass) != -1 && soldierXp >= minXp) {
